...
const teacherModel = require('../Models/landOwner');
const ManagerModel = require('../Models/reseacher');
...
const LandOwnerController = {

  getAllLandOwners: asyncWrapper(async (req, res, next) => {
    const users = await teacherModel.find({})
    res.status(200).json({ users })
  }),
  getLandOwnersbyId: asyncWrapper(async (req, res, next) => {
    const { id: userID } = req.params
    const user = await teacherModel.findOne({ _id: userID });
    if (!user) {
      return next(new Notfound(`LandOwner not found`));
    };
  ...
  OTP: asyncWrapper(async (req, res, next) => {
    ...
    if (savedUser) {
      return res.status(201).json({
        message: "LandOwner account verified!",
        user: savedUser
      });
  ...
  UpdatePassword: asyncWrapper(async (req, res, next) => {
    ...
    if (!user) {
      return next(new Notfound(`Teacher not found`));
    }
